...
image_dir = Path("Generated Image Path") #The image path generated by the model
output_path = Path("Output Results Path") #Detailed path of the output evaluation results
temp_file =  Path("Temp Image Path")
...
def process_sample(idx: int, metadata: dict, max_retries: int = 6) -> dict:
        ...
        with open(temp_file, "r", encoding="utf-8") as f:
            for line in f:
                try:
                    ...
                    if data["result"].get("correct", 0) == 1:
                        tag_counter[tag]["correct"] += 1

                    if "id" in data: 
                        processed_indices.add(data["id"])
                except json.JSONDecodeError:
                    continue  

    print(f"Processed {len(processed_indices)} , start from {len(processed_indices)}.")


    for idx, metadata in tqdm(enumerate(lines), total=len(lines), desc="Evaluating"):
        if idx+1 in processed_indices:
            continue  
