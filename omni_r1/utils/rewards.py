...
sys.path.append("/root/paddlejob/workspace/env_run/output/projects/X-Omni-RL")
...
def pickscore_score(device):
    from utils.pickscore_scorer import PickScoreScorer
...
def imagereward_score(device):
    from utils.imagereward_scorer import ImageRewardScorer
...
def qwenvl_score(device):
    from utils.qwenvl import QwenVLScorer
...
def ocr_score(device):
    from utils.ocr import OcrScorer
...
def video_ocr_score(device):
    from utils.ocr import OcrScorer_video_or_image
...
def prob_score(device):
    system_prompts="Describe the image in detail."
    ...
    def _fn(images,prompts, metadata):
        ...
        with torch.no_grad():
            for i, (prompt, image) in enumerate(zip(prompts,images)):
                ...
                gt_ids= tokenizer.encode(prompt)
                gt_ids= torch.tensor(gt_ids,dtype=torch.int64,device=device).unsqueeze(0)
                #print(gt_ids)
                all_probs=1.0
                current_input_ids=input_ids.to(model.device)
                for i in range(len(gt_ids[0])):
                    outputs=model(current_input_ids).logits
                    last_token_logits = outputs[:, -1, :]
                    probs = F.softmax(last_token_logits, dim=-1)
                    gt_token_prob = probs[:, gt_ids[: ,i]].item()
                    #print(gt_token_prob)
                    #all_probs.append(gt_token_prob)
                    all_probs *= gt_token_prob
                    current_input_ids = torch.cat([current_input_ids, gt_ids[:,i].view(1, -1)], dim=-1)
...
def gvu_score(images,prompts, metadata):
    system_prompts="Describe the image in detail."
    results=[]
    model = metadata.get('model')
    model.set_generation_mode('text')
    ...
    with torch.no_grad():
        for i, (prompt, image) in enumerate(zip(prompts,images)):
            ...
            input_ids = tokenizer.apply_chat_template(message, add_generation_prompt=True, return_tensors='pt')
            gt_ids= tokenizer.encode(prompt)
            gt_ids= torch.tensor(gt_ids,dtype=torch.int64,device=model.device).unsqueeze(0)
            #print(gt_ids)
            all_probs=1.0
            current_input_ids=input_ids.to(model.device)
            for i in range(len(gt_ids[0])):
                ...
                all_probs *= gt_token_prob
                current_input_ids = torch.cat([current_input_ids, gt_ids[:,i].view(1, -1)], dim=-1)
            all_probs = all_probs ** (1.0/gt_ids.size(1))
            results.append(all_probs)
    model.set_generation_mode('image')
    return results   
...
def test_prob_score():
    ...
    tokenizer = AutoTokenizer.from_pretrained("/root/paddlejob/workspace/env_run/models/X-Omni-En", use_fast=True)
    model = AutoModelForCausalLM.from_pretrained(
        "/root/paddlejob/workspace/env_run/models/X-Omni-En", 
        torch_dtype=torch_dtype,
        trust_remote_code=True,
    ).cuda()
    model.init_vision("/root/paddlejob/workspace/env_run/models/FLUX.1-dev")
    model.set_generation_mode('text')
    model.eval()
    image = Image.open("/root/paddlejob/workspace/env_run/output/projects/X-Omni/test.png").convert('RGB')
    image=[image]
    prompt= ["A dog playing water."]

    score_fn=prob_score(model.device)
    result=score_fn(image,prompt,metadata={"model":model, "tokenizer":tokenizer })
