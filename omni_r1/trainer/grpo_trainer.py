...
class XOmniGRPOTrainer(Trainer):
    """
    ...
    def __init__(
        ...
        self.image_prefix = f'<SOM>{token_h} {token_w}<IMAGE>'
        ...
        self.generation_config = GenerationConfig(
            ...
            suppress_tokens=self.processing_class.convert_tokens_to_ids(model.config.mm_special_tokens),
        )
        self.beta = args.beta
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        prompt_inputs=self.processing_class(
            [prompt+ self.image_prefix for prompt in prompts],
            return_tensors='pt', 
            padding='longest', 
            padding_side='left',
        )
        ...
        negative_ids = self.processing_class.encode(
            self.image_prefix, 
        ...
        is_eos= completion_ids == self.processing_class.convert_tokens_to_ids('<EOM>')
        ...
        prompt_completion_ids= torch.nn.functional.pad(prompt_completion_ids, (0, 1), value=self.processing_class.convert_tokens_to_ids('<EOM>'))
        images = []
        with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
            for i in range(prompt_completion_ids.shape[0]):  
                current_input = prompt_completion_ids[i].unsqueeze(0)  # (1, L)
                #current_input = current_input.to("cpu")
                #current_input = current_input.to("cpu")
                unwrapped_model.decoder_pipe.to(device)
                current_input.to(device)
                _, current_image = unwrapped_model.mmdecode(
                    self.processing_class,
                    current_input,
                    skip_special_tokens=False
                )
                # Save Results
                images.append(current_image[0])  
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, PreTrainedModel):
                ...
                with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
                    ...
                    metadata={"model": unwrapped_model, "tokenizer":self.processing_class}
                    output_reward_func = reward_func(images=images, prompts=prompts, metadata=metadata)
